...
- (instancetype) initWithSplashIcon:(SKSplashIcon *)icon backgroundImage:(UIImage *)backgroundImage animationType:(SKSplashAnimationType)animationType
{
    ...
    if(self)
    {
        ...
        imageView.frame = self.bounds;
        [self addSubview:imageView];
        icon.frame = CGRectMake(imageView.frame.size.width/2-icon.iconSize.width/2, imageView.frame.size.height/2 - icon.iconSize.height/2, icon.iconSize.width,icon.iconSize.height);
        [self addSubview:icon];
    }
    return self;
}
